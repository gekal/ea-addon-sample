@echo off
rem ============================================================
rem EAAddinSetup.bat バージョン3.4.3
rem ============================================================
rem パラメータ
rem なし : アドインインストール
rem /u   : アドインアンインストール
...
rem ■■ レジストリ登録・削除 ■■
:regImport
set RegistoryPath="HKCU\Software\Sparx Systems\EAAddins"\%AddinName%
...
IF not "%RegValue%"=="-" (
 IF %SetupKind%==add (
  %RegExeFolder%\reg.exe add %RegistoryPath% /ve /f /d %RegValue%) ELSE (
   IF %delCounter%==1 (%RegExeFolder%\reg.exe DELETE %RegistoryPath% /f)
   IF not %errorlevel%==0 (
